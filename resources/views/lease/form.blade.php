...
<div class="form-group {{ $errors->has('RoomID') ? 'has-error' : ''}}">
    ...
    onchange="showMeter_Electricity(),showMeter_Water(),leasemaxid()">
        ...
        @foreach($rooms->where('RoomStatus', 'ว่าง') as $room)
        @php
...
<div class="form-group {{ $errors->has('Usersid') ? 'has-error' : ''}}">
    <label for="Usersid" class="control-label">{{ 'ชื่อผู้เช่า' }}</label>
    <select class="form-control" name="Usersid" id="Usersid">
        <option value="">เลือกผู้เช่า</option>
        @foreach($users as $user) 
            @php
                $hasLease = App\Models\Lease::where('Usersid', $user->id)->exists();
            @endphp
            @if (!$hasLease)
                <option value="{{ $user->id }}" {{ isset($lease->Usersid) && $lease->Usersid == $user->id ? 'selected' : '' }}>
                    {{ $user->name }}
                </option>
            @endif
        @endforeach
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    roomIDInput.addEventListener("change", function() {
        ...
        // ตรวจสอบค่า roomID ก่อนที่จะดึงข้อมูลจาก $rooms
        if (roomID) {
            var selectedRoom = {!! json_encode($rooms->keyBy('id')->toArray()) !!}[roomID];

            // ตรวจสอบว่า selectedRoom ไม่เป็น null และมี field Room_Price และตัวแปร Room_Price ไม่เป็น null ก่อนที่จะกำหนดค่าให้กับ input field
            if (selectedRoom && selectedRoom.hasOwnProperty('Room_Price') && selectedRoom.Room_Price) {
                roomPriceInput.value = selectedRoom.Room_Price;
            } else {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    roomIDInput.addEventListener("change", function() {
        ...
        if (roomID) {
            ...
            // ตรวจสอบว่า selectedRoom ไม่เป็น null และมี field Booking_Price และตัวแปร Booking_Price ไม่เป็น null ก่อนที่จะกำหนดค่าให้กับ input field
            if (selectedRoom && selectedRoom.hasOwnProperty('Booking_Price') && selectedRoom.Booking_Price) {
                bookingPriceInput.value = selectedRoom.Booking_Price;
            } else {
                bookingPriceInput.value = "";
            }
        } else {
            bookingPriceInput.value = "";
        }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    roomIDInput.addEventListener("change", function() {
        ...
        if (roomID) {
            ...
            // ตรวจสอบว่า selectedRoom ไม่เป็น null และมี field Deposit_Price และตัวแปร Deposit_Price ไม่เป็น null ก่อนที่จะกำหนดค่าให้กับ input field
            if (selectedRoom && selectedRoom.hasOwnProperty('Deposit_Price') && selectedRoom.Deposit_Price) {
                depositPriceInput.value = selectedRoom.Deposit_Price;
            } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add event listener to the dropdown RoomID
    var roomIDInput = document.getElementById('RoomID');
    roomIDInput.addEventListener('change', function() {
        // Call the function to recalculate Net_Pay and LeaseTotal when the dropdown changes
        calculateNetPayAndLeaseTotal();
    });
});
</script>
...
<div class="form-group {{ $errors->has('Lease_Doc') ? 'has-error' : ''}}">
    <label for="Lease_Doc" class="control-label">{{ '' }}</label>
    <input class="form-control" name="Lease_Doc" type="hidden" id="Lease_Doc"
